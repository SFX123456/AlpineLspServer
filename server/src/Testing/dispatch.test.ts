...
import {isInsideDispatchSetEvent} from "../methods/textDocument/completion/completionJs";
...
describe('dispatchtest',() => {
    ...
    test(' check if it parsed right dispatched event with number', () => {
        ...
        setUpFiles();
        const wholeline = "   $dispatch('                   "
        const res = isInsideDispatchSetEvent(wholeline,14)
